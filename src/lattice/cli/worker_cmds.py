...
def _load_worker_def(workers_dir: Path, name: str) -> dict | None:
    """Load a worker definition by name (case-insensitive filename match)."""
...
def _create_worktree(project_root: Path, task_id: str, commit_sha: str, event_id: str) -> Path:
    ...
    worktree_path = Path(f"/tmp/lattice-worker-{short_task}-{short_sha}-{short_eid}")
...
def worker_run(worker_name: str, task_id: str, actor: str | None, is_json: bool) -> None:
    ...
    responsible for posting process_completed/process_failed.
    """
    ...
    # Write process_started event and update snapshot
    ...
    try:
        ...
        # Spawn failed — post process_failed event
        ...
        failed_snapshot = apply_event_to_snapshot(updated_snapshot, failed_event)
        with multi_lock(lattice_dir / "locks", lock_keys):
            jsonl_append(event_path, serialize_event(failed_event))
            atomic_write(
                lattice_dir / "tasks" / f"{task_id}.json",
                serialize_snapshot(failed_snapshot),
            )
        if worktree_path:
