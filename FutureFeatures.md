...
- **Lifecycle:** When a branch merges and the task is marked `done`, the node dims or retracts — satisfying, accumulative, potentially emergent.
...
**Cardinality:** Many-to-many. A task can link to branches in multiple repos (cross-repo feature). Multiple tasks can converge on one integration branch.
